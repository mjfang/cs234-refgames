...
#Generalize to more than one.

class SpeakerListenerN:
...
#agnostic: 0.01
  ...
  def init_speaker(self, informed = True, hidden_size=100, num_filters = 40, vocab_size=10, is_train = True):

    self.x_target = tf.placeholder(tf.float32, (None, self.in_size))
    self.x_distractor = tf.placeholder(tf.float32, (None, self.in_size))
    self.x_distractor2 = tf.placeholder(tf.float32, (None, self.in_size))
    ...
    #informed
      if informed:
        ...
        h_distractor = layers.fully_connected(self.x_distractor, hidden_size, activation_fn=None, scope=scope,
                                              reuse=True)
        h_distractor2 = layers.fully_connected(self.x_distractor2, hidden_size, activation_fn=None, scope=scope,
                                              reuse=True)
        x = tf.expand_dims(tf.stack([h_target, h_distractor, h_distractor2], axis=2), -1) # (batch, hidden, 2)
        x_conv = tf.layers.conv2d(x, num_filters, [1,3], activation=tf.sigmoid, use_bias=False) # (batch, hidden, 20)
  ...
  def init_listener(self, hidden_size = 100, is_train = True):

    self.x_1 = tf.placeholder(tf.float32, (None, self.in_size))
    self.x_2 = tf.placeholder(tf.float32, (None, self.in_size))
    self.x_3 = tf.placeholder(tf.float32, (None, self.in_size))
    ...
    with tf.variable_scope("listener") as scope:
      ...
      h_3 = layers.fully_connected(self.x_3, hidden_size, activation_fn = None, scope=scope, reuse=True)
    ...
    dot3 = tf.diag_part(tf.matmul(h_3, sym_embed))

    self.out = tf.stack([dot1, dot2, dot3], axis=1) / self.temp  #batch_size, 2
